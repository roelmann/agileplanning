...
<header class="pageheader jumbotron text-center">
    ...
    <div class="filtercontrols container">
        <form action="" method="post" class="row">
            <div class="filterby filterbydate col">
                <label><strong>Sprint: </strong></label><br>
                <input type="number" name="sprint" min="1" max="5000">
            </div>
    ...
    if (isset($_POST['sprint']) && $_POST['sprint'] !== '') {
        $sprintdatesql = "SELECT * FROM weeks WHERE sprint = " . $_POST['sprint'];
        $sprintdate = $db_handle->runQuery($sprintdatesql);
        $startdate = $enddate = '';
        foreach ($sprintdate as $k => $v) {
            if ($startdate == '') {
                $startdate = $sprintdate[$k]['weekcommencing']; // If statement keeps this as the first set.
            }
    ...
    $sql2 = "SELECT CONCAT_WS(';',taskid,developerid) FROM " . $table . " WHERE developerid = " . $devid . " ORDER BY developerid";
    echo $sql2;
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                foreach($efforttd as $eff) {
                    ...
                    // Dev.
                    $sqldev = "SELECT firstname, lastname FROM developers WHERE id = " . $devid;
                    $dev = $db_handle->runQuery($sqldev);
                    foreach ($dev as $d){
